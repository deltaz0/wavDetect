...
valInd = 37 # (0 to 80)
seqL = 200
pklFileNorm = open('../nCoder/ncoderdata/featuresNormalNonDecodedExtract.pkl', 'rb')
pklFileRook = open('../nCoder/ncoderdata/featuresRookNonDecodedExtract.pkl', 'rb')
fNorm = pickle.load(pklFileNorm, encoding='bytes')
fRook = pickle.load(pklFileRook, encoding='bytes')
pklFileNorm.close()
pklFileRook.close()
...
for i in range(100):
    ...
    xTrain = np.roll(fTotalTrain,rI,axis=0).reshape(-1,seqL,fTotalTrain.shape[1])
    yTrain = np.roll(fTotalTarget,rI,axis=0).reshape(-1,seqL,fTotalTrain.shape[1])
    model.fit(xTrain,yTrain,batch_size=np.random.randint(10,50),shuffle=False,epochs=10)
model.save('modelTotalTrainRandomizedDecE.h5')
del model
...
from keras.models import Model, load_model
model = load_model('modelTotalTrainRandomizedDecE.h5')
l2NormRMSNorm = []
l2NormRMSRook = []
for i in range(3):
