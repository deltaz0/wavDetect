...
maxs = np.zeros(128)
with open('../nCoder/ncoderforknew/Project3/myOutNewNonDecodedNew.csv', 'r') as csvfile:
...
plt.show()
maxs = np.zeros(253)
with open('../nCoder/ncoderforknew/Project3/myOutNewNonDecoded.csv', 'r') as csvfile:
    cList = csv.reader(csvfile)
    for i,o in enumerate(cList):
        maxs[i] = len(o)
plt.plot(maxs)
